                        ...
                        children: [
                          ...
                          BlocBuilder<GenreCubit, GenreState>(
                            builder: (context, genreState) {
                              if (genreState is GenreLoading || genreState is GenreInitial) {
                                ...
                                final genres = genreState.genres; // True genres from the state
                                if (genres.isEmpty) {
                                ...
                                return Center(
                                  child: Wrap(
                                    ...
                                    children: genres.map((genre) {
                                      final isSelected = tempFavoriteGenres.contains(genre.toUpperCase());
                                      return GestureDetector(
                                        onTap: () {
                                          setState(() {
                                            if (isSelected) {
                                              tempFavoriteGenres.remove(genre.toUpperCase());
                                            } else {
                                              if (tempFavoriteGenres.length < 3) {
                                                tempFavoriteGenres.add(genre.toUpperCase());
                                              } else {
                                                ScaffoldMessenger.of(context).showSnackBar(
                                                  const SnackBar(
                                        ...
                                        child: Container(
                                          height: 8.h, // Adjusted height for bigger size
                                          width: 20.w, // Approx. 4 items per row
                                          decoration: isSelected
                                              ? BoxDecoration(
                                                  gradient: generateHashGradient(decorations[genre.toUpperCase()]!),
                          ...
                          Center(
                            child: CustomElevatedButton(
                              ...
                              height: 50,
                              decoration: BoxDecoration(
                                color: tempFavoriteGenres.length < 3 ? Colors.grey.shade300 : Colors.orange,
                              ...
                              buttonTextStyle: TextStyle(
                                color: tempFavoriteGenres.length < 3 ? Colors.grey : Colors.white,
                                fontSize: 16,
